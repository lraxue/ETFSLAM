...
//        LOG(INFO) << "ext: " << ext;
    ...
    cv::Mat System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp)
    {
        ...
        mTrackedKeyPointsUn = mpTracker->mCurrentFrame.mvKeysLeft;
        return Tcw;
    }
    ...
    {
        cout << endl << "Save Map to " << filename << endl;
        ...
        for (auto &pKF : vpKFs)
        {
            file << pKF->mnId << " " << !pKF->isBad() << " " << pKF->N << endl;
            ...
            for (int i = 0; i < N; ++i)
            {
                ...
                if (pMP && !pMP->isBad())
                {
                    file << 1 << " " << pMP->mnId << " " << pMP->GetWorldPos().at<float>(0) << " "
                         << pMP->GetWorldPos().at<float>(1) << " " << pMP->GetWorldPos().at<float>(2) << " "
                         << pKF->ComputeReprojectionError(i) << endl;
                }
                else
                {
