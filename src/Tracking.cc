...
//            mpLocalMapper->ProcessKeyFrame();
    ...
    void Tracking::MonocularInitialization()
    {
        ...
        {
            // Set Reference Frame
            if (mCurrentFrame.mvKeysLeft.size() > 100)
            {
                mInitialFrame = Frame(mCurrentFrame);
                mLastFrame = Frame(mCurrentFrame);
                mvbPrevMatched.resize(mCurrentFrame.mvKeysLeft.size());
                for (size_t i = 0; i < mCurrentFrame.mvKeysLeft.size(); i++)
                    mvbPrevMatched[i] = mCurrentFrame.mvKeysLeft[i].pt;
        ...
        {
            // Try to initialize
            if ((int) mCurrentFrame.mvKeysLeft.size() <= 100)
            {
                delete mpInitializer;
...
//        LOG(INFO) << "UpdateLastFrame.";
    ...
    bool Tracking::TrackWithMotionModel()
    {
        ...
        if (mSensor != System::STEREO)
            ...
            th = 7;
        int nmatches = matcher.SearchByProjectionWithCircleMatch(mCurrentFrame, mLastFrame, th);
//        int nmatches = matcher.SearchByProjection(mCurrentFrame, mLastFrame, th, mSensor == System::MONOCULAR);
        ...
        {
            fill(mCurrentFrame.mvpMapPoints.begin(), mCurrentFrame.mvpMapPoints.end(), static_cast<MapPoint *>(NULL));
            nmatches = matcher.SearchByProjectionWithCircleMatch(mCurrentFrame, mLastFrame, 2 * th);
//            nmatches = matcher.SearchByProjection(mCurrentFrame, mLastFrame, 2 * th, mSensor == System::MONOCULAR);
    ...
    // TODO
    void Tracking::CreateNewKeyFrameOnStereo()
    {
        KeyFrame *pKF = new KeyFrame(mCurrentFrame, mpMap, mpKeyFrameDB);
...
//            sort(vWeightIdx.begin(), vWeightIdx.end());
//
//            int nW = vWeightIdx.size() - 1;
//            for (int i = 0; i < 200; ++i)
//            {
//                int nDepthIdx = vDepthIdx[i].second;
//                int nWeightIdx = vWeightIdx[nW - i].second;
//                LOG(INFO) << "Depth: " << nDepthIdx << " " << vDepthIdx[i].first << " " << mCurrentFrame.mvpTriangles[nDepthIdx]->mFusedUncertainty
//                          << " Weight: " << nWeightIdx << " " << mCurrentFrame.mvDepth[nWeightIdx] << " " << vWeightIdx[nWeightIdx].first;
//            }
...
//        mpLocalMapper->SetNotStop(false);
        mpLocalMapper->ProcessKeyFrame();    // New Added function
