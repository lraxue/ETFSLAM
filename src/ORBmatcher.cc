    ...
    while(KFit != KFend && Fit != Fend)
    {
        if(KFit->first == Fit->first)
        {
            ...
            for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
            {
                ...
                const cv::Mat &dKF= pKF->mDescriptorsLeft.row(realIdxKF);
                ...
                if(bestDist1<=TH_LOW)
                {
                    if(static_cast<float>(bestDist1)<mfNNratio*static_cast<float>(bestDist2))
                    {
                        ...
                        const cv::KeyPoint &kp = pKF->mvKeysLeft[realIdxKF];

                        if(mbCheckOrientation)
                        {
                            float rot = kp.angle-F.mvKeysLeft[bestIdxF].angle;
                            if(rot<0.0)
                                rot+=360.0f;
...
int ORBmatcher::SearchByProjection(KeyFrame* pKF, cv::Mat Scw, const vector<MapPoint*> &vpPoints, vector<MapPoint*> &vpMatched, int th)
{
    ...
    for(int iMP=0, iendMP=vpPoints.size(); iMP<iendMP; iMP++)
    {
        ...
        for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
        {
            ...
            const int &kpLevel= pKF->mvKeysLeft[idx].octave;
...
{
    int nmatches=0;
    vnMatches12 = vector<int>(F1.mvKeysLeft.size(),-1);
    ...
    vector<int> vMatchedDistance(F2.mvKeysLeft.size(),INT_MAX);
    vector<int> vnMatches21(F2.mvKeysLeft.size(),-1);

    for(size_t i1=0, iend1=F1.mvKeysLeft.size(); i1<iend1; i1++)
    {
        cv::KeyPoint kp1 = F1.mvKeysLeft[i1];
        ...
        if(bestDist<=TH_LOW)
        {
            if(bestDist<(float)bestDist2*mfNNratio)
            {
                ...
                if(mbCheckOrientation)
                {
                    float rot = F1.mvKeysLeft[i1].angle-F2.mvKeysLeft[bestIdx2].angle;
                    if(rot<0.0)
                        rot+=360.0f;
                    int bin = round(rot*factor);
                    if(bin==HISTO_LENGTH)
    ...
    for(size_t i1=0, iend1=vnMatches12.size(); i1<iend1; i1++)
        if(vnMatches12[i1]>=0)
            vbPrevMatched[i1]=F2.mvKeysLeft[vnMatches12[i1]].pt;
...
{
    const vector<cv::KeyPoint> &vKeysUn1 = pKF1->mvKeysLeft;
    const DBoW2::FeatureVector &vFeatVec1 = pKF1->mFeatVec;
    const vector<MapPoint*> vpMapPoints1 = pKF1->GetMapPointMatches();
    const cv::Mat &Descriptors1 = pKF1->mDescriptorsLeft;

    const vector<cv::KeyPoint> &vKeysUn2 = pKF2->mvKeysLeft;
    const DBoW2::FeatureVector &vFeatVec2 = pKF2->mFeatVec;
    const vector<MapPoint*> vpMapPoints2 = pKF2->GetMapPointMatches();
    const cv::Mat &Descriptors2 = pKF2->mDescriptorsLeft;
    ...
    while(f1it!=f1end && f2it!=f2end)
    {
        if(f1it->first == f2it->first)
        {
            for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
            {
                ...
                const cv::KeyPoint &kp1 = pKF1->mvKeysLeft[idx1];
                
                const cv::Mat &d1 = pKF1->mDescriptorsLeft.row(idx1);
                ...
                for(size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                {
                    ...
                    const cv::Mat &d2 = pKF2->mDescriptorsLeft.row(idx2);
                    ...
                    const cv::KeyPoint &kp2 = pKF2->mvKeysLeft[idx2];
                ...
                if(bestIdx2>=0)
                {
                    const cv::KeyPoint &kp2 = pKF2->mvKeysLeft[bestIdx2];
                             ...
                             const float &s12, const cv::Mat &R12, const cv::Mat &t12, const float th)
{
    ...
    for(int i1=0; i1<N1; i1++)
    {
        ...
        for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
        {
            ...
            const cv::KeyPoint &kp = pKF2->mvKeysLeft[idx];

            if(kp.octave<nPredictedLevel-1 || kp.octave>nPredictedLevel)
                continue;

            const cv::Mat &dKF = pKF2->mDescriptorsLeft.row(idx);
    ...
    for(int i2=0; i2<N2; i2++)
    {
        ...
        for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
        {
            ...
            const cv::KeyPoint &kp = pKF1->mvKeysLeft[idx];

            if(kp.octave<nPredictedLevel-1 || kp.octave>nPredictedLevel)
                continue;

            const cv::Mat &dKF = pKF1->mDescriptorsLeft.row(idx);
...
int ORBmatcher::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono)
{
        ...
        if(pMP)
        {
            if(!LastFrame.mvbOutlier[i])
            {
                ...
                if(bestDist<=TH_HIGH)
                {
                    ...
                    if(mbCheckOrientation)
                    {
                        float rot = LastFrame.mvKeysLeft[i].angle-CurrentFrame.mvKeysLeft[bestIdx2].angle;
                        if(rot<0.0)
                            rot+=360.0f;
                        int bin = round(rot*factor);
                        if(bin==HISTO_LENGTH)
    ...
    int ORBmatcher::SearchByProjectionWithCircleMatch(Frame &CurrentFrame, const Frame &LastFrame, const float th)
    {
        int nmatches = 0;
        ...
        const float &bf = CurrentFrame.mbf;
...
//                        continue;
                    ...
                    if (u < 0 || u >= CurrentFrame.mnMaxX)
                        continue;
                    if (v < 0 || v >= CurrentFrame.mnMaxY)
                        continue;
        ...
        if(pMP)
        {
            if(!pMP->isBad() && !sAlreadyFound.count(pMP))
            {
                ...
                if(bestDist<=ORBdist)
                {
                    ...
                    if(mbCheckOrientation)
                    {
                        float rot = pKF->mvKeysLeft[i].angle-CurrentFrame.mvKeysLeft[bestIdx2].angle;
                        if(rot<0.0)
                            rot+=360.0f;
                        int bin = round(rot*factor);
                        if(bin==HISTO_LENGTH)
