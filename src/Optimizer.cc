...
namespace ORB_SLAM2
{
                                     ...
                                     int nIterations, bool *pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
    {
        ...
        for (size_t i = 0; i < vpMP.size(); i++)
        {
            ...
            for (map<KeyFrame *, size_t>::const_iterator mit = observations.begin(); mit != observations.end(); mit++)
            {
                ...
                const cv::KeyPoint &kpUn = pKF->mvKeysLeft[mit->second];
    ...
    void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool *pbStopFlag, Map *pMap)
    {
        ...
        for (list<MapPoint *>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end(); lit != lend; lit++)
        {
                 ...
                 mit != mend; mit++)
            {
                ...
                if (!pKFi->isBad())
                {
                    const cv::KeyPoint &kpUn = pKFi->mvKeysLeft[mit->second];
                                ...
                                const float th2, const bool bFixScale)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            const cv::KeyPoint &kpUn1 = pKF1->mvKeysLeft[i];
            ...
            const cv::KeyPoint &kpUn2 = pKF2->mvKeysLeft[i2];
    ...
    void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, Map *pMap)
    {
        ...
        {
            std::map<KeyFrame *, size_t> obs = (*vit)->GetObservations();
        ...
        for (std::vector<KeyFrame *>::iterator vit = vLocalFixedKFs.begin(), vend = vLocalFixedKFs.end();
             vit != vend; vit++)
        {
            KeyFrame *pKFi = *vit;
            g2o::VertexSE3Expmap *vSE3 = new g2o::VertexSE3Expmap();
        ...
        for (std::vector<MapPoint *>::iterator vit = vLocalMPs.begin(), vend = vLocalMPs.end(); vit != vend; vit++)
        {
            ...
            // Set edges
            for (std::map<KeyFrame *, size_t>::const_iterator mit = obs.begin(), mend = obs.end(); mit != mend; mit++)
            {
                KeyFrame *pKFi = mit->first;
        ...
        LOG(INFO) << "Erase failed observations and recover optimized parameters.";
...
//        std::unique_lock<std::mutex> lock(pMap->mMutexMapUpdate);
        ...
        LOG(INFO) << "Begin to recover optimized parameters.";
