...
namespace ORB_SLAM2
{
    ...
    KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB) :
            ...
            mDescriptorRight(F.mDescriptorsRight.clone()),
    ...
    float KeyFrame::ComputeReprojectionError(const int &idx)
    {
        ...
        const cv::Mat &X3Dw = mvpMapPoints[idx]->GetWorldPos();
        const cv::Mat X3Dc = Tcw.rowRange(0, 3).colRange(0, 3) * X3Dw + Tcw.rowRange(0, 3).col(3);

        const float x = X3Dc.at<float>(0);
        const float y = X3Dc.at<float>(1);
        const float invz = 1.0 / X3Dc.at<float>(2);

        const float ru = x * fx * invz + cx;
        const float rv = y * fy * invz + cy;

        const float du = mvKeysLeft[idx].pt.x - ru;
        const float dv = mvKeysLeft[idx].pt.y - rv;
